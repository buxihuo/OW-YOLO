...
def non_max_suppression(prediction,
    ...
    prediction[..., 85:86] *= (prediction[..., 4:5] > 0.5)
    xc = (prediction[..., 4] > conf_thres) + (prediction[..., 85] > conf_thres) # candidates
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Compute conf
        x[:, 5:85] *= x[:, 4:5]  # conf = obj_conf * cls_conf
        fconf, fj = x[:, 86:97].max(1, keepdim=True)
        x[:, 86:97] = 0
        ...
        if multi_label:
            i, j = (x[:, 5:97] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float()), 1)
        else:  # best class only
            conf, j = x[:, 5:97].max(1, keepdim=True)
